...
<script>
import {getSysMsg ,getMessage} from "../../api/api";
            ...
            let dataRequest = {
              ...
              getSysMsg(dataRequest).then((response) => {
                ...
                this.dataList = response.data;
                 ...
                 getMessage(dataRequest).then((response) => {
                console.log('我是getSysMsg返回数据',response);
                this.url = response.data.userhead;
                });
        },
        methods:{
                ...
                this.socket.onmessage = function(msg) {
                    ...
                    if(msg.data&&_this.isJSON(msg.data)) {
                        ...
                        // _this.username = name
                            ...
                            getMessage(dataRequest2).then((response) => {
                            console.log('我是getSysMsg2返回数据',response);
                            this.url2 = response.data.userhead;
                            window.localStorage.setItem("head2",this.url2);
                            });
                        const obj = {
                            content: contentText,
                            //head: toUserId == this.userId? this.url : this.url2,
                             head: toUserId == this.userId? this.url : 'https://rollup.oss-cn-hangzhou.aliyuncs.com/user_head/default.png',    
